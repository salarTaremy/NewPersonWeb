...
{
    public class CustomUserStore : IUserStore<ApplicationUser>, IUserPasswordStore<ApplicationUser> ,IQueryableUserStore<ApplicationUser>, IDisposable
    {
        ...
        public async Task<ApplicationUser> FindByIdAsync(string userId, CancellationToken cancellationToken)
        {
            using (var conn = _dbConnection.GetConnection)
            {
                var query = $"SELECT  Code_melli as id , FullName , Code_melli as UserName ,  WebPassword    " +
                    $",[Email]= null,[EmailConfirmed] = CAST(0 AS BIT),[PasswordHash] = null ,[SecurityStamp]= null ,[PhoneNumber] = null,[PhoneNumberConfirmed] = CAST(0 AS BIT) ,[TwoFactorEnabled]  = CAST(0 AS BIT),[LockoutEndDateUtc] = null ,[LockoutEnabled]=CAST(0 AS BIT),[AccessFailedCount] = CAST(0 AS int)" +
                    $" FROM [th].[Taraf_Hesab] WHERE [Code_melli] = @Id";
                var param = new DynamicParameters();
                param.Add("@Id", userId);
        ...
        public async Task<ApplicationUser> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken)
        {
            using (var conn = _dbConnection.GetConnection)
            {
                var query = $"SELECT   Code_melli as id , FullName , Code_melli as UserName  , WebPassword   " +
                    $",[Email]= null,[EmailConfirmed] = CAST(0 AS BIT),[PasswordHash] = null ,[SecurityStamp]= null ,[PhoneNumber] = null,[PhoneNumberConfirmed] = CAST(0 AS BIT) ,[TwoFactorEnabled]  = CAST(0 AS BIT),[LockoutEndDateUtc] = null ,[LockoutEnabled]=CAST(0 AS BIT),[AccessFailedCount] = CAST(0 AS int)" +
                    $" FROM [th].[Taraf_Hesab] WHERE [Code_melli] = @normalizedUserName";
                var param = new DynamicParameters();
